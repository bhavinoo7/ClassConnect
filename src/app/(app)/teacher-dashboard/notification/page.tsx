...
export default function Page() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get("/api/noti-fetch-teacher?teacher_id=" + teacherid)

      setNotificat(response.data.data.notification)
      dispatch(NotificationActions.setNotification(response.data.data.notification))
      dispatch(NotificationActions.setUnread(response.data.data.unread))
  ...
  const sentByTeacherNotifications = notifications.filter((n) => n.source === "teacher")
  const hodNotifications = notifications.filter((n) => n.source === "hod")

  const teacherSentUnreadCount = sentByTeacherNotifications.filter((n) => !n.read).length
  ...
  const deleteNotification = (id: string) => {
    setNotificat((prev) => prev.filter((notification) => notification.id !== id))
  }
  ...
  const filteredTeacherSentNotifications = filterNotifications(sentByTeacherNotifications)
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <Tabs
        ...
        className="w-full"
        onValueChange={(value) => setCurrentRole(value as "teacher" | "student")}
      >
        ...
        <TabsContent value="teacher">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                <CardTitle>Teacher Sent Notifications</CardTitle>
                <CardDescription>Notifications sent by teachers with recipient information</CardDescription>
              </div>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Notifications from HODs to teachers</CardDescription>
              </div>
            </CardHeader>
            <CardContent>
            ...
            <>
              <DialogHeader>
                ...
                <DialogDescription className="flex justify-between items-center mt-2">
                  ...
                  <span>
                    {formatDate(selectedNotification.timestamp)} at {formatTime(selectedNotification.timestamp)}
                  </span>
                </DialogDescription>
              </DialogHeader>
              ...
              {selectedNotification.recipients && selectedNotification.recipients.length > 0 && (
                <div className="mt-4 border-t pt-4">
                  ...
                  <div className="bg-muted p-3 rounded-md">
                    <p className="text-sm">{selectedNotification.recipients.join(", ")}</p>
                  </div>
                </div>
              )}
