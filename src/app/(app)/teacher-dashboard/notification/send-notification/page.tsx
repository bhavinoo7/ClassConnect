...
export default function Page() {
  ...
  const { teacherid, name } = useAppSelector((state) => state.user);
  const [shouldFetch, setShouldFetch] = useState<boolean>(true);
  const { isLoading, data } = useQuery({
    queryKey: ["fetch-class"],
    queryFn: async () => {
      const response = await axios.get(
        `/api/noti-class-fetch?teacher_id=${teacherid}`
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post("/api/noti-save", { teacherid, obj });
      ...
      <div className="mb-8">
        ...
        >
          <TabsList className="grid w-full grid-cols-2 mb-8">
            <TabsTrigger value="teacher">Send as Teacher</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="teacher">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  As a Teacher, you can send notifications to students in your
                  class.
                </CardDescription>
              </CardHeader>
