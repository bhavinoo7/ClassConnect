...
const [enrollmentData, setEnrollmentData] = useState<{ session_id?: string; session_name?: string; date?: string; time?: string }[]>([]);
...
useEffect(()=>{
  if (sub1) {
    const newData = sub1
      .map((s: { session_id: string[]; session_name: string; date: string }) => ({
        session_id: s.session_id[0],
        session_name: s.session_name,
        date: s.date.split("T")[0],
        time: s.date.split("T")[1],
      }))
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()); // ✅ Sorting based on date change a to b for ascending order and b to a for descending order
  ...
  const filteredData = enrollmentData.filter((enrollment) => {
    ...
    return (
      ...
      (enrollment.session_name?.toLowerCase().includes(searchString) ?? false) ||
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className=" md:table-cell">View attendance</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {currentItems.length > 0 ? (
                  currentItems.map((enrollment) => (
                    <TableRow key={enrollment.session_id}>
                      <TableCell className="font-medium">{enrollment.session_id}</TableCell>
                      <TableCell>{enrollment.session_name}</TableCell>
                      <TableCell>{enrollment.date}</TableCell>
                      <TableCell>{enrollment.time}</TableCell>
                      <TableCell className=" md:table-cell text-green-400">Completed</TableCell>
                      <TableCell className=" md:table-cell"><Button onClick={()=>router.replace(`/division-dashboard/sub/${params.sid}/${enrollment.session_id}?teacherid=${teacher_id}&subjectid=${subject_id}`)}>view Attendance</Button></TableCell>
          ...
          {filteredData.length > 0 && (
            <div className="mt-4 flex items-center justify-between">
              <div className="text-sm text-muted-foreground">
                ...
                {indexOfLastItem > filteredData.length ? filteredData.length : indexOfLastItem} of {filteredData.length}{" "}
                entries
              </div>
              <Pagination>
                <PaginationContent>
