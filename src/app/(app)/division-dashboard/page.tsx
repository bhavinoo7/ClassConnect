...
export default function Page() {
  ...
  const currentSemesterData =
    semesterData[selectedSemester as keyof typeof semesterData];
  ...
  function calculateProgress(start_date: any, end_date: any) {
    ...
    const totalDuration =
      (endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24); // Total days
    const daysPassed =
      (today.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24); // Days passed
  ...
  useEffect(() => {
    fetch("/api/get-cookie")
      ...
      .then((data) => {
        ...
        dispatch(
          timeTableActions.setdivisionid(data.divisionSession.division_id)
        );
      });
  }, []);
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await axios.get(
        `/api/fetch-division-data?division_id=${sessionData}`
      );
      ...
      const semData = response.data.data.map((se: any) => {
        return {
          ...
          progress: calculateProgress(
            se.semester_start_date.split("T")[0],
            se.semester_end_date.split("T")[0]
          ),
      ...
      setSemesterData(
        Object.assign({}, ...semData.map((s: any) => ({ [s.id]: s })))
      );
      setSemesters(sem);
    }
    if (sessionData) {
      ...
      <main className="flex-1 p-6 md:p-8">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {currentSemesterData?.students}
              </div>
              <p className="text-xs text-muted-foreground">
                Active students in this division
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {currentSemesterData?.teachers}
              </div>
              <p className="text-xs text-muted-foreground">
                Faculty assigned to this division
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Subjects taught in this division
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {currentSemesterData?.progress.toString().split(".")[0]}%
              </div>
              <Progress
                value={currentSemesterData?.progress}
                className="h-2 mt-2"
              />
              <p className="text-xs text-muted-foreground mt-2">
                {currentSemesterData?.startDate} -{" "}
                {currentSemesterData?.endDate}
              </p>
            </CardContent>
          </Card>
        </div>
                        ...
                        paginatedSubjects.map((subject) => (
                          <TableRow key={uuidv4()}>
                            ...
                            <TableCell className="text-right">
                              {subject?.total_completed_session}
                            </TableCell>
                            <TableCell className="text-right">
                              <Button
                                variant="outline"
                  ...
                  {totalSubjectPages > 1 && (
                    <Pagination className="mt-4">
                      <PaginationContent>
                        ...
                        {Array.from({ length: totalSubjectPages }).map(
                          (_, i) => (
                            <PaginationItem key={i}>
                              <PaginationLink
                                onClick={() => setSubjectPage(i + 1)}
                                isActive={subjectPage === i + 1}
                              >
                                {i + 1}
                              </PaginationLink>
                            </PaginationItem>
                        ...
                        <PaginationItem>
                          <PaginationNext
                            onClick={() =>
                              setSubjectPage((p) =>
                                Math.min(totalSubjectPages, p + 1)
                              )
                            }
                            className={
                          ...
                          <TableRow key={uuidv4()}>
                            ...
                            <TableCell className="text-right">
                              <Button
                                ...
                                onClick={() =>
                                  router.replace(
                                    `division-dashboard/${student?.id}sm${selectedSemester}`
                                  )
                                }
                              >
                  ...
                  {totalStudentPages > 1 && (
                    <Pagination className="mt-4">
                      <PaginationContent>
                        ...
                        {Array.from({ length: totalStudentPages }).map(
                          (_, i) => (
                            <PaginationItem key={i}>
                              <PaginationLink
